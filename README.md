...
**Important:** Do NOT use `uv run train.py` — this project does not use a `pyproject.toml` for dependency management. Always use `.venv/bin/python train.py`. The venv must be set up manually with the commands below.

```bash
# Create venv and install deps (one-time)
# NOTE: Must use Python 3.12 (not 3.10). The venv has no pip by default (uv venvs skip it).
# Use `uv pip install` commands below — do NOT use `.venv/bin/pip` directly.
...
For training on a cloud GPU (e.g., RTX PRO 6000 96GB):
...
Set `HF_UPLOAD = True` and `HF_REPO_ID` in `train.py`, then train. After training, download the GGUF on your local machine:

```bash
huggingface-cli download <HF_REPO_ID> --local-dir ./model --include "gguf/*"
# Create Modelfile, then:
...
- **NVIDIA DGX Spark**: GB10 GPU, 128GB unified memory, CUDA 13.0 (development)
- **RunPod RTX PRO 6000**: 96GB VRAM (cloud training)
